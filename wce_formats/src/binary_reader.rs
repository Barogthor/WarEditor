...
impl BinaryReader {
    ...
    pub fn read_c_string(&mut self) -> ReadResult<CString> {
        ...
        self.buffer
            .read_until('\0' as u8, &mut result_buf)
            .map_err(|e| to_read_error(self, e))?;
        result_buf.pop();
    ...
    pub fn read_version<T: BinaryConverterVersion>(
        &mut self,
    ...
    pub fn read_vec_version<T: BinaryConverterVersion>(
        &mut self,
        size: usize,
        game_version: &GameVersion,
    ) -> ReadResult<Vec<T>> {
    ...
    pub fn seek_begin(&mut self) {
        self.buffer
            ...
            .expect("Failed to move cursor at start.");
    }

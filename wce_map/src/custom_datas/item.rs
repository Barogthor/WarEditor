...
impl CustomItemFile {
    pub fn read_file(map: &mut MapArchive, game_version: &GameVersion) -> Result<Option<Self>, OpeningError>{
        let file = map.open_file(MAP_CUSTOM_ITEMS);
        match file {
            Ok(file) => {
                let mut buffer: Vec<u8> = vec![0; file.size() as usize];

                file.read(map, &mut buffer).map_err(|e| OpeningError::CustomItem(format!("{}",e)))?;
    ...
    fn from(reader: &mut BinaryReader, game_version: &GameVersion) -> Self {
        ...
        assert_eq!(reader.size(), reader.pos() as usize, "reader for {} hasn't reached EOF. Missing {} bytes", MAP_CUSTOM_ITEMS, reader.size() - reader.pos() as usize);
        Self {
            version,
            original_objects,
            custom_objects

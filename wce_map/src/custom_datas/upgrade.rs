...
impl CustomUpgradeFile {
    pub fn read_file(map: &mut MapArchive, game_version: &GameVersion) -> Result<Option<Self>, OpeningError>{
        let file = map.open_file(MAP_CUSTOM_UPGRADES);
        match file {
            Ok(file) => {
                let mut buffer: Vec<u8> = vec![0; file.size() as usize];

                file.read(map, &mut buffer).map_err(|e| OpeningError::CustomUpgrade(format!("{}",e)))?;
                let mut reader = BinaryReader::new(buffer);
                let custom_upgrade = Self::from(&mut reader, game_version).map_err(|e| OpeningError::CustomUpgrade(format!("{:?}", e)))?;

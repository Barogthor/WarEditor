...
impl CustomBuffFile {
    pub fn read_file(mpq: &mut Archive, data: &GameData) -> Self{
        let file = mpq.open_file(MAP_CUSTOM_BUFFS);
        match file {
            Ok(file) => {
    ...
    fn from(reader: &mut BinaryReader, data: &GameData) -> Self {
        ...
        assert_eq!(reader.size(), reader.pos() as usize, "reader for {} hasn't reached EOF. Missing {} bytes", MAP_CUSTOM_BUFFS, reader.size() - reader.pos() as usize);
        Self {
            version,
            original_objects,
            custom_objects

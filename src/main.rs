...
use war_editor::globals::{SLK_UNIT_META_DATA, PROFILE_ITEM_FUNC, PROFILE_ITEM_ABILITY_FUNC, PROFILE_UNDEAD_UNIT_FUNC, PROFILE_ORC_UNIT_FUNC, PROFILE_NIGHT_ELF_UNIT_FUNC, PROFILE_NEUTRAL_UNIT_FUNC, PROFILE_HUMAN_UNIT_FUNC, PROFILE_CAMPAIGN_UNIT_FUNC, PROFILE_HUMAN_ABILITY_FUNC, PROFILE_ORC_ABILITY_FUNC, PROFILE_NIGHT_ELF_ABILITY_FUNC, PROFILE_UNDEAD_ABILITY_FUNC, PROFILE_NEUTRAL_ABILITY_FUNC, PROFILE_CAMPAIGN_ABILITY_FUNC, PROFILE_COMMON_ABILITY_FUNC, PROFILE_HUMAN_UPGRADE_FUNC, PROFILE_ORC_UPGRADE_FUNC, PROFILE_UNDEAD_UPGRADE_FUNC, PROFILE_NIGHT_ELF_UPGRADE_FUNC, PROFILE_NEUTRAL_UPGRADE_FUNC, PROFILE_CAMPAIGN_UPGRADE_FUNC, PROFILE_UNIT_EDITOR_DATA, PROFILE_WORLD_EDIT_STRINGS, PROFILE_WORLD_EDIT_LAYOUT, PROFILE_WORLD_EDIT_DATA, PROFILE_WORLD_EDIT_GAME_STRINGS, PROFILE_WAR3SKINS, PROFILE_MISC_DATA, PROFILE_AIEDITOR_DATA, SLK_DOODAD_META_DATA, SLK_DESTRUCTABLE_META_DATA, SLK_ABILITY_META_DATA, SLK_UPGRADE_META_DATA, SLK_UPGRADE_EFFECT_META_DATA, SLK_MISC_META_DATA, SLK_SKIN_META_DATA, SLK_ABILITY_BUFF_META_DATA, SLK_UNIT_DATA, SLK_UNIT_COMBAT_SOUNDS, SLK_UNIT_UI, SLK_UNIT_ABILITIES, SLK_UNIT_BALANCE, SLK_UNIT_WEAPONS, SLK_ABILITY_DATA, SLK_UPGRADE_DATA, SLK_DESTRUCTABLE_DATA, SLK_DOODADS};
...
fn main() {
    ...
    ini.fit();
//    ini.debug();
//    let unit_meta = SLKData::open(&format_slk(SLK_UNIT_META_DATA));
//    let doodad_meta = SLKData::open(&format_slk(SLK_DOODAD_META_DATA));
//    let destructable_meta = SLKData::open(&format_slk(SLK_DESTRUCTABLE_META_DATA));
//    let abilty_meta = SLKData::open(&format_slk(SLK_ABILITY_META_DATA));
//    let upgrade_meta = SLKData::open(&format_slk(SLK_UPGRADE_META_DATA));
//    let upgrade_effect_meta = SLKData::open(&format_slk(SLK_UPGRADE_EFFECT_META_DATA));
//    let const_meta = SLKData::open(&format_slk(SLK_MISC_META_DATA));
//    let ui_const_meta = SLKData::open(&format_slk(SLK_SKIN_META_DATA));
//    let ability_buff_meta = SLKData::open(&format_slk(SLK_ABILITY_BUFF_META_DATA));
...
//    let mut ability_data = SLKData::open(&format_slk(SLK_ABILITY_DATA));
//    let mut upgrade_data = SLKData::open(&format_slk(SLK_UPGRADE_DATA));
//    let mut doodad_effect_data = SLKData::open(&format_slk(SLK_DOODADS));
//    let mut destructable_effect_data = SLKData::open(&format_slk(SLK_DESTRUCTABLE_DATA));
